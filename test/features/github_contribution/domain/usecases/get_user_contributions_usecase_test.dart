...
import 'package:git_hub_contribution_app/features/github_contribution/domain/usecases/get_user_contributions_usecase.dart';
import '../../../../fixtures/test_data.dart';

import 'get_user_contributions_usecase_test.mocks.dart';
...
void main() {
  ...
  group('GetUserContributionsUseCase', () {
    late GetUserContributionsUseCase useCase;
    late MockGithubRepository mockRepository;

    setUp(() {
      mockRepository = MockGithubRepository();
      useCase = GetUserContributionsUseCase(mockRepository);
    });

    group('正常系', () {
      test('有効なtoken、username、yearでContributionリストを取得できる', () async {
        // Arrange
        ...
        when(
          mockRepository.getUserContributions(token, username, year),
        ).thenAnswer((_) async => Right(contributions));

        // Act
        final result = await useCase.call(token, username, year);
